...
def prepare(date):
    try:
        ...
        print(f"indicators_data_daily_job.prepare处理异常：{e}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_data):
                ...
                try:
                    ...
                    print(f"indicators_data_daily_job.run_check处理异常：{stock[1]}代码{e}")
    except Exception as e:
        print(f"indicators_data_daily_job.run_check处理异常：{e}")
    if not data:
        return None
    else:
        return data
...
def guess_buy(date):
    try:
        ...
        print(f"indicators_data_daily_job.guess_buy处理异常：{e}")
...
def guess_sell(date):
    try:
        ...
        print(f"indicators_data_daily_job.guess_sell处理异常：{e}")
