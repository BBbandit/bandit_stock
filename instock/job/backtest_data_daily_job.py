...
def process(table, data_all, date, backtest_column):
    ...
    try:
        ...
        print(f"backtest_data_daily_job.process处理异常：{table}表{e}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_data):
                ...
                try:
                    ...
                    print(f"backtest_data_daily_job.run_check处理异常：{stock[1]}代码{e}")
    except Exception as e:
        print(f"backtest_data_daily_job.run_check处理异常：{e}")
    if not data:
        return None
    else:
        return data
