...
def prepare(date, strategy):
    try:
        ...
        print(f"strategy_data_daily_job.prepare处理异常：{strategy}策略{e}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_data):
                ...
                try:
                    ...
                    print(f"strategy_data_daily_job.run_check处理异常：{stock[1]}代码{e}策略{table_name}")
    except Exception as e:
        print(f"strategy_data_daily_job.run_check处理异常：{e}策略{table_name}")
    if not data:
        return None
    else:
        return data
