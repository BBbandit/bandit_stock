...
def get_connection():
    try:
        ...
        print(f"database.conn_not_cursor处理异常：{MYSQL_CONN_DBAPI}{e}")
    return None
...
def insert_other_db_from_df(to_db, data, table_name, cols_type, write_index, primary_keys, indexs=None):
    ...
    # 清理和转换 change_rate 数据
    if 'change_rate' in data.columns:
        data['change_rate'] = pd.to_numeric(data['change_rate'], errors='coerce')
        data = data.dropna(subset=['change_rate'])  # 去掉无效数据
    ...
    try:
        ...
        print(f"database.insert_other_db_from_df处理异常：{table_name}表{e}")
    ...
    with get_connection() as conn:
        with conn.cursor() as db:
            try:
                ...
                print(f"database.update_db_from_df处理异常：{sql}{e}")
    ...
    with get_connection() as conn:
        with conn.cursor() as db:
            try:
                ...
                print(f"database.executeSql处理异常：{sql}{e}")
    ...
    with get_connection() as conn:
        with conn.cursor() as db:
            try:
                ...
                print(f"database.executeSqlFetch处理异常：{sql}{e}")
    return None
    ...
    with get_connection() as conn:
        with conn.cursor() as db:
            try:
                ...
                print(f"database.select_count计算数量处理异常：{e}")
    return 0
